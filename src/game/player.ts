...
export class Player {
  ...
  playCombo(combo: Hand, lastHandPlayed: Hand | undefined, round: number) {
    ...
    if (round === 0 && !lastHandPlayed && !combo.has(Deck.LOWEST_CARD)) {
      return {
        validCombo: false,
        comboPlayed: undefined,
  ...
  playBestHand(lastHandPlayed: Hand | undefined, round: number) {
    ...
    if (isRoundLeader) {
      const mustPlayThreeOfDiamonds =
        isRoundLeader && round === 0 && this.has(Deck.LOWEST_CARD);
      if (mustPlayThreeOfDiamonds) {
        bestHandForRound = Object.values(this.combos)
          .flat()
          .find((s) => s.hand.has(Deck.LOWEST_CARD))?.hand;
      } else {
        // otherwise decide whether to play the best overall hand or one that just barely beats the last player
        const bestCombo = Object.keys(this.combos).find((k) => {
          const subcombos = this.combos[k as keyof typeof COMBOS];
          return subcombos && subcombos.length > 0;
        });
        const bestComboValues: SubCombo[] =
          this.combos[bestCombo as keyof typeof COMBOS] || [];
      ...
      // this means we have to follow the pattern already set by the prev player
      bestHandForRound = this.bestHand(
        COMBOS[lastHandPlayed.type as keyof typeof COMBOS],
        lastHandPlayed
      );
    }
    ...
    if (
      !isRoundLeader &&
      lastHandPlayed &&
      !bestHandForRound.beats(lastHandPlayed)
    ) {
      this.skipRound();
      return;
    }
  ...
  bestHand(type: COMBOS, lastHandPlayed: Hand) {
    ...
    if (type === COMBOS.FULL_HAND) {
      const moves = availableMoves
        .map((c) => c.hand)
        .filter((h) => h.beats(lastHandPlayed));
    ...
    const bestCards = shuffle([
      ...new Set(
        availableMoves
          .filter((m) => m.hand.beats(lastHandPlayed))
          .map((m) => parseInt(m.rank))
      ),
    ]);
    ...
    if (randomlyDecideToSkip) {
      console.log(
        `${this.name} wants to save some of their cards for later...`
      );
      return;
    }
  ...
  eligibleMoves(lastComboPlayed: Hand): string[] {
    const keys = Object.keys(this.combos)
      .filter((k) => lastComboPlayed.type == k)
      .map((k) => this.combos[k as keyof typeof COMBOS])
      .flat();
    if (!keys) {
      return [];
    }
    return keys
      .reduce((prev: Card[], curr) => {
  ...
  logCombos(lastComboPlayed: Hand | undefined) {
    ...
    const subcombos =
      keys
        .filter((k) => this.combos[k] && this.combos[k].length > 0)
        .map((k) => this.combos[k])
    ...
    const groups = groupBy(
      subcombos.map((s) => {
        return {
          combo: s.type,
          hands:
            s.hand.cards.length === 5
              ? s.hand.toString()
              : [...new Set(s.hand.rank())].pop(),
        };
      }),
      (c) => c.combo
    );
    const rows = Object.entries(groups).map(([type, vals]) => {
      return { combo: type.replace("_", " "), hands: vals.map((v) => v.hands) };
    });
    const table = new Table({
      columns: [
        { name: "combo", color: "yellow" },
        { name: "hands", color: "green" },
      ],
    ...
    const singles: SubCombo[] = Object.entries(handCount)
      ...
      .map(([rank]) => {
        return new SubCombo(
          rank,
          new Hand(
            this.hand.cards.filter((c) => c.rank === parseInt(rank)).slice(0, 1)
          ),
          COMBOS.SINGLE
        );
      });
    ...
    const pairs = Object.entries(handCount)
      ...
      .map(([rank]) => {
        return new SubCombo(
          rank,
          new Hand(
            this.hand.cards.filter((c) => c.rank === parseInt(rank)).slice(0, 2)
          ),
          COMBOS.PAIR
        );
      });
    ...
    const tripleCombos = Object.entries(handCount)
      ...
      .map(([rank]) => {
        return new SubCombo(
          rank,
          new Hand(
            this.hand.cards.filter((c) => c.rank === parseInt(rank)).slice(0, 3)
          ),
          COMBOS.TRIPLE
        );
      });
  ...
  getFullHandCombos(existingCombos: {
    [type in COMBOS]?: SubCombo[];
  }): SubCombo[] {
    const fullHand: SubCombo[] = [];
    const [, [type, { isValid: isValidSequentialCombo }]] = Object.entries(
      FullHandCombo
