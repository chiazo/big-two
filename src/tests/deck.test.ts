...
describe("deck.test.ts", () => {
    const lowestCard = new Card(
        CardSuit.DIAMONDS.toString(),
        CardSuit.DIAMONDS.symbol,
        3
    );
    const highestCard = new Card(
        CardSuit.SPADES.toString(),
        CardSuit.SPADES.symbol,
        2
    );
    ...
    describe("use cases", () => {
        it("returns standard shuffled deck", () => {
            ...
            assert.notStrictEqual(
                first(deck.cards)?.toString(),
                first(originalCardOrder)?.toString()
            );
        ...
        it("returns standard unshuffled deck", () => {
            ...
            const expectedFirst = new Card(
                CardSuit.DIAMONDS.toString(),
                CardSuit.DIAMONDS.symbol,
                RANKS.NUMERAL_CARDS.MIN.value
            );
            const expectedLast = new Card(
                CardSuit.SPADES.toString(),
                CardSuit.SPADES.symbol,
                RANKS.FACE_CARDS.KING
            );
