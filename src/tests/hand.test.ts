...
describe("hand.test.ts", () => {
  describe("use cases", () => {
    ...
    describe("full house", () => {
      it("verifies random hand", () => {
        verifyHand(COMBOS.FULL_HAND, FULL_HAND_TYPES.FULL_HOUSE, (c, h) =>
          Object.entries(calculateHandCount(h)).some(
            ([key, val]) => val === 3 && parseInt(key) === c.rank
          )
        );
      });
      it("beats a straight", () => {
  ...
  describe("edge cases", () => {
    it("throws if max called on empty hand", () => {
      assert.throws(
        () => new Hand([]).max(),
        (err: Error) => {
          assert.strictEqual(err.message, "max is invalid for this hand.");
          return true;
        }
      );
    });
    it("throws if max called on empty full house hand", () => {
      assert.throws(
        ...
        (err: Error) => {
          assert.strictEqual(
            err.message,
            "max full house is invalid for this hand."
          );
          return true;
        }
      );
    ...
    it("throws if join exceeds hand card limit", () => {
      assert.throws(
        () => {
          getDesiredHand(new Deck(), COMBOS.TRIPLE)?.join(
            getDesiredHand(new Deck(), COMBOS.TRIPLE)
          );
        },
        (err: Error) => {
          assert.strictEqual(
            err.message,
            "cannot merge hands and exceed 5 cards"
          );
          return true;
        }
      );
    ...
    it("throws if join involves overlapping cards", () => {
      assert.throws(
        ...
        (err: Error) => {
          assert.strictEqual(
            err.message,
            "cannot merge hands that have overlapping cards"
          );
          return true;
        }
      );
...
const verifyHandBeatsAnother = (c: CardCombo) => {
  ...
  const result =
    ace && king && two && ace.beats(king) && two.beats(ace) && two.beats(king);
  assert(result);
};

const verifyFullHandBeatsAnother = (
  a: FULL_HAND_TYPES,
  b: FULL_HAND_TYPES | undefined = undefined
): void => {
  const deck = new Deck();
  ...
  if (b) {
    const one =
      a === FULL_HAND_TYPES.FULL_HOUSE
        ? getSpecificHandAndRank(deck, CardCombo.TRIPLE, 13)?.join(
            getDesiredHand(deck, COMBOS.PAIR)
          )
        : getDesiredHand(deck, c, a, 2);
    const two =
      b === FULL_HAND_TYPES.FULL_HOUSE
        ? getSpecificHandAndRank(deck, CardCombo.TRIPLE, 12)?.join(
            getDesiredHand(deck, COMBOS.PAIR)
          )
        : getDesiredHand(deck, c, b, 2);
    assert(one && two && one.beats(two));
    return;
  }
  const ace =
    a === FULL_HAND_TYPES.FULL_HOUSE
      ? getSpecificHandAndRank(deck, CardCombo.TRIPLE, 1)?.join(
          getDesiredHand(deck, COMBOS.PAIR)
        )
      : getDesiredHand(deck, c, a, 1);
  const king =
    b === FULL_HAND_TYPES.FULL_HOUSE
      ? getSpecificHandAndRank(deck, CardCombo.TRIPLE, 13)?.join(
          getDesiredHand(deck, COMBOS.PAIR)
        )
      : getDesiredHand(deck, c, a, 13);
